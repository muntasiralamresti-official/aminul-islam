...
export default function BatchesPage() {
  ...
  const fetchBatches = useCallback(async ({ force = false } = {}) => {
    ...
    if (!hasDataRef.current && !hasCachedData) setLoading(true);
  ...
  return (
    <div className="w-full pb-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex w-full items-center gap-3 sm:w-auto"><Link href="/dashboard/batches/new" className="inline-flex min-h-10 w-full items-center justify-center rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 sm:w-auto"><Plus className="mr-2 h-4 w-4" /> Add Batch</Link></div>
      </div>

      <div className="mt-6 hidden w-full overflow-hidden rounded-xl shadow-sm ring-1 ring-black/5 sm:mt-8 sm:block"><div className="w-full overflow-x-auto overscroll-x-contain"><table className="w-full min-w-[760px] table-fixed divide-y divide-gray-200"><colgroup><col className="w-[24%]" /><col className="w-[22%]" /><col className="w-[16%]" /><col className="w-[18%]" /><col className="w-[10%]" /><col className="w-[10%]" /></colgroup><thead className="bg-gray-50"><tr><th className="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-6">Name</th><th className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Subject</th><th className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Class</th><th className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Schedule</th><th className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Capacity</th><th className="relative py-3.5 pl-3 pr-4 sm:pr-6"><span className="sr-only">Actions</span></th></tr></thead><tbody className="divide-y divide-gray-100 bg-white">{batches.map((batch) => <tr key={batch._id} className="transition-colors hover:bg-gray-50/80"><td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6"><div className="truncate">{batch.name}</div></td><td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500"><div className="truncate">{batch.subject}</div></td><td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500"><div className="truncate">{batch.classLevel}</div></td><td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500"><div className="truncate">{batch.schedule}</div></td><td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{batch.capacity}</td><td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6"><Link href={`/dashboard/batches/${batch._id}/edit`} className="mr-1 inline-flex min-h-9 min-w-9 items-center justify-center rounded-lg text-blue-600 hover:bg-blue-50 hover:text-blue-900 sm:mr-2" aria-label={`Edit ${batch.name}`}><Edit className="h-4 w-4" /></Link><button onClick={() => setDeleteId(batch._id)} className="inline-flex min-h-9 min-w-9 items-center justify-center rounded-lg text-red-600 hover:bg-red-50 hover:text-red-900" aria-label={`Delete ${batch.name}`}><Trash2 className="h-4 w-4" /></button></td></tr>)}{batches.length === 0 && <tr><td colSpan="6" className="py-14 text-center"><div className="mx-auto max-w-sm"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-blue-50 text-blue-600"><Plus className="h-6 w-6" /></div><p className="mt-3 text-sm font-medium text-gray-900">No batches found</p><p className="mt-1 text-sm text-gray-500">Create your first batch to get started.</p></div></td></tr>}</tbody></table></div></div>

      <div className="mt-6 space-y-3 sm:hidden">
        {batches.map((batch) => (
          <article key={batch._id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_8px_26px_rgba(15,23,42,0.07)]">
            <div className="border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white px-4 py-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <h2 className="truncate text-base font-bold text-slate-900">{batch.name}</h2>
                  <p className="mt-1 truncate text-sm font-medium text-blue-600">{batch.subject || 'No subject'}</p>
                </div>
                <span className="flex-shrink-0 rounded-full bg-blue-50 px-2.5 py-1 text-xs font-bold text-blue-700">Class {batch.classLevel || '—'}</span>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-px bg-slate-100">
              <div className="bg-white px-4 py-3"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Schedule</p><p className="mt-1 line-clamp-2 text-sm font-semibold text-slate-700">{batch.schedule || 'Not set'}</p></div>
              <div className="bg-white px-4 py-3"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Capacity</p><p className="mt-1 text-sm font-semibold text-slate-700">{batch.capacity ?? '—'} students</p></div>
            </div>
            <div className="flex gap-2 border-t border-slate-100 bg-slate-50 p-2.5">
              <Link href={`/dashboard/batches/${batch._id}/edit`} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-white text-sm font-semibold text-blue-700 shadow-sm ring-1 ring-slate-200"><Edit className="h-4 w-4" /> Edit</Link>
              <button onClick={() => setDeleteId(batch._id)} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-white text-sm font-semibold text-red-600 shadow-sm ring-1 ring-slate-200"><Trash2 className="h-4 w-4" /> Delete</button>
            </div>
          </article>
        ))}
        {batches.length === 0 && <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-blue-50 text-blue-600"><Plus className="h-6 w-6" /></div><p className="mt-3 text-sm font-medium text-gray-900">No batches found</p><p className="mt-1 text-sm text-gray-500">Create your first batch to get started.</p></div>}
      </div>

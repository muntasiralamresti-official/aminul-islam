...
export default function FeesPage() {
  ...
  const openNewPaymentModal = (studentId = "") => {
    ...
    setModalStudentSearch(""); setShowStudentDropdown(false); setShowModal(true);
  };
  ...
  return <div className="space-y-5 pb-28 sm:space-y-6 sm:pb-8">
    ...
    {showModal && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-3 sm:p-4" role="dialog" aria-modal="true"><div className="relative max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl sm:p-6"><button type="button" onClick={() => !saving && setShowModal(false)} className="absolute right-3 top-3 rounded-lg p-2 text-gray-400 hover:bg-gray-100" aria-label="Close"><X className="h-5 w-5" /></button><div className="pr-10"><p className="text-xs font-semibold uppercase tracking-wide text-blue-600">{editMode ? "Update record" : "New collection"}</p><h2 className="mt-1 text-xl font-bold text-gray-900">{editMode ? "Edit Payment" : "Record Payment"}</h2><p className="mt-1 text-xs text-gray-500">Select the student, amount, payment date and method.</p></div><form onSubmit={handleSubmit} className="mt-5 space-y-4"><div><label className="mb-1.5 block text-sm font-semibold text-gray-700">Student</label><div className="relative"><input type="text" placeholder="Search by name or roll number..." value={showStudentDropdown ? modalStudentSearch : (selectedStudentObj ? `${selectedStudentObj.name} (${selectedStudentObj.rollNumber})` : "")} onChange={(event) => { setModalStudentSearch(event.target.value); setShowStudentDropdown(true); if (!event.target.value) setFormData((current) => ({ ...current, student: "" })); }} onFocus={() => { setShowStudentDropdown(true); setModalStudentSearch(""); }} className="w-full rounded-xl border border-gray-300 bg-white px-3 py-3 text-sm text-gray-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" autoComplete="off" />{showStudentDropdown && filteredModalStudents.length > 0 && <div className="absolute z-50 mt-1 max-h-56 w-full overflow-y-auto rounded-xl border border-gray-200 bg-white shadow-xl">{filteredModalStudents.map((student) => <button key={student._id} type="button" onMouseDown={() => { setFormData((current) => ({ ...current, student: student._id, amount: student.currentDue > 0 ? student.currentDue : (student.monthlyFee || "") })); setModalStudentSearch(""); setShowStudentDropdown(false); }} className={`flex w-full items-center justify-between gap-3 px-3 py-3 text-left text-sm hover:bg-blue-50 ${formData.student === student._id ? "bg-blue-50 font-semibold text-blue-700" : "text-gray-900"}`}><span>{student.name}</span><span className="text-xs text-gray-400">Roll: {student.rollNumber}</span></button>)}</div>}{showStudentDropdown && filteredModalStudents.length === 0 && <div className="absolute z-50 mt-1 w-full rounded-xl border border-gray-200 bg-white px-3 py-3 text-sm text-gray-400 shadow-xl">No student found.</div>}</div><input type="hidden" required value={formData.student} onChange={() => {}} />{selectedStudentSummary && <div className="mt-2 grid grid-cols-3 gap-2 rounded-xl bg-gray-50 p-3 text-xs text-gray-600"><div>Monthly<br /><strong className="text-gray-900">{money(selectedStudentSummary.monthlyFee)}</strong></div><div>Current due<br /><strong className="text-blue-700">{money(selectedStudentSummary.currentDue)}</strong></div><div>Previous due<br /><strong className="text-amber-700">{money(selectedStudentSummary.previousDue)}</strong></div></div>}</div><div className="grid grid-cols-2 gap-3"><div><label className="mb-1.5 block text-sm font-semibold text-gray-700">Month</label><select value={formData.month} onChange={(event) => setFormData((current) => ({ ...current, month: event.target.value }))} className="w-full rounded-xl border border-gray-300 bg-white px-3 py-3 text-sm text-gray-900">{MONTHS.map((month) => <option key={month}>{month}</option>)}</select></div><div><label className="mb-1.5 block text-sm font-semibold text-gray-700">Year</label><input type="number" min="2000" max="2100" value={formData.year} onChange={(event) => setFormData((current) => ({ ...current, year: Number(event.target.value) }))} className="w-full rounded-xl border border-gray-300 px-3 py-3 text-sm text-gray-900" /></div></div><div className="grid grid-cols-2 gap-3"><div><label className="mb-1.5 block text-sm font-semibold text-gray-700">Amount</label><input required min="1" step="0.01" type="number" value={formData.amount} onChange={(event) => setFormData((current) => ({ ...current, amount: event.target.value }))} placeholder="e.g. 1000" className="w-full rounded-xl border border-gray-300 px-3 py-3 text-sm text-gray-900" /></div><div><label className="mb-1.5 block text-sm font-semibold text-gray-700">Payment Date</label><input required type="date" value={formData.date} onChange={(event) => setFormData((current) => ({ ...current, date: event.target.value }))} className="w-full rounded-xl border border-gray-300 px-3 py-3 text-sm text-gray-900" /></div></div><div className="grid grid-cols-2 gap-3"><div><label className="mb-1.5 block text-sm font-semibold text-gray-700">Payment Method</label><select value={formData.method} onChange={(event) => setFormData((current) => ({ ...current, method: event.target.value }))} className="w-full rounded-xl border border-gray-300 bg-white px-3 py-3 text-sm capitalize text-gray-900">{PAYMENT_METHODS.map((method) => <option key={method} value={method}>{method}</option>)}</select></div><div><label className="mb-1.5 block text-sm font-semibold text-gray-700">Status</label><select value={formData.status} onChange={(event) => setFormData((current) => ({ ...current, status: event.target.value }))} className="w-full rounded-xl border border-gray-300 bg-white px-3 py-3 text-sm text-gray-900"><option value="paid">Paid</option><option value="due">Due / Not Collected</option></select></div></div><div className="flex flex-col-reverse gap-2 border-t border-gray-100 pt-4 sm:flex-row sm:justify-end"><button type="button" disabled={saving} onClick={() => setShowModal(false)} className="min-h-11 rounded-xl border border-gray-200 px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50">Cancel</button><button type="submit" disabled={saving} className="inline-flex min-h-11 min-w-40 items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white disabled:cursor-wait disabled:opacity-70">{saving && <RefreshCw className="h-4 w-4 animate-spin" />}{saving ? "Saving..." : editMode ? "Update Payment" : "Record Payment"}</button></div></form></div></div>}

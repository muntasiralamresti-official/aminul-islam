  ...
  return <div className="min-h-dvh bg-gray-100 flex">
    <div className={clsx("fixed inset-0 z-50 lg:hidden", sidebarOpen ? "pointer-events-auto" : "pointer-events-none")} role="dialog" aria-modal="true" aria-hidden={!sidebarOpen}>
      ...
      <div className={clsx("relative flex h-full w-[min(86vw,320px)] flex-col bg-white pt-[max(1rem,env(safe-area-inset-top))] pb-4 shadow-2xl transition-transform duration-200 ease-out", sidebarOpen ? "translate-x-0" : "-translate-x-full")}>
        ...
        <div className="h-px bg-gray-100" /><div className="mt-4 flex-1 overflow-y-auto px-3"><nav className="space-y-1.5">{visibleNavigation.map(item => { const active = pathname === item.href || (item.href !== "/dashboard" && pathname.startsWith(`${item.href}/`)); return <Link key={item.name} href={item.href} onClick={() => haptic()} className={clsx(active ? "bg-blue-600 text-white shadow-sm" : "text-gray-600 hover:bg-gray-50", "group flex min-h-12 items-center rounded-xl px-4 py-3 text-[15px] font-semibold")}><item.icon className={clsx(active ? "text-white" : "text-gray-400", "mr-4 h-5 w-5")} />{item.name}</Link>; })}<InstallButton mobile /></nav></div>

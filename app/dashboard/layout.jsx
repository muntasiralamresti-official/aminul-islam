  ...
  return <div className="min-h-screen bg-gray-100 flex">
    <div className={clsx("fixed inset-0 z-50 lg:hidden", sidebarOpen ? "pointer-events-auto" : "pointer-events-none")} role="dialog" aria-modal="true" aria-hidden={!sidebarOpen}>
      ...
      <div className={clsx("relative flex h-full w-[min(86vw,320px)] flex-col bg-white pt-[max(1rem,env(safe-area-inset-top))] pb-4 shadow-2xl transition-transform duration-200 ease-out", sidebarOpen ? "translate-x-0" : "-translate-x-full")}>
        <div className="flex flex-shrink-0 items-center justify-between px-5 pb-5"><div className="flex min-w-0 items-center"><div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-blue-600 shadow-sm"><GraduationCap className="h-6 w-6 text-white" /></div><span className="ml-3 truncate text-lg font-bold text-gray-900">{centerName}</span></div><button type="button" className="ml-3 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200 hover:text-gray-900" onClick={() => { haptic(); setSidebarOpen(false); }}><span className="sr-only">Close menu</span><X className="h-5 w-5" aria-hidden="true" /></button></div>
    ...
    <div className="min-w-0 min-h-0 flex flex-1 flex-col overflow-hidden">
      <div className="safe-top flex min-h-14 flex-shrink-0 items-center justify-between border-b border-gray-200 bg-white/95 px-3 shadow-sm backdrop-blur lg:hidden"><div className="flex min-w-0 items-center"><div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl bg-blue-600"><GraduationCap className="h-5 w-5 text-white" /></div><div className="ml-2.5 min-w-0"><p className="truncate text-[10px] font-semibold uppercase tracking-wider text-blue-600">Coaching Center</p><p className="truncate text-sm font-bold text-gray-900">{centerName}</p></div></div><button type="button" className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-gray-50 text-gray-600 hover:bg-gray-100 hover:text-gray-900" onClick={() => { haptic(); setSidebarOpen(true); }}><span className="sr-only">Open menu</span><Menu className="h-5 w-5" aria-hidden="true" /></button></div>
      <header className="flex-shrink-0 border-b border-gray-200 bg-white px-3 py-2.5 sm:px-6 lg:px-8 lg:py-3"><form onSubmit={handleSearch} className="relative mx-auto max-w-xl"><Search className="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-400" aria-hidden="true" /><input type="search" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search students by name, roll, phone, or batch..." aria-label="Search students" className="block min-h-11 w-full rounded-xl border border-gray-200 bg-gray-50 py-2.5 pl-10 pr-4 text-sm text-gray-900 shadow-none outline-none transition focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100" /></form></header>

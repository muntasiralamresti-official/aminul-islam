  ...
  return <div className="min-h-screen bg-gray-100 flex">
    ...
    <div className={clsx("fixed inset-0 z-50 lg:hidden", sidebarOpen ? "pointer-events-auto" : "pointer-events-none")} role="dialog" aria-modal="true" aria-hidden={!sidebarOpen}>
      ...
      <div className={clsx("relative flex h-full w-[min(86vw,320px)] flex-col bg-white pt-[max(1rem,env(safe-area-inset-top))] pb-4 shadow-2xl transition-transform duration-200 ease-out", sidebarOpen ? "translate-x-0" : "-translate-x-full")}>
        <div className="flex flex-shrink-0 items-center justify-between px-5 pb-5">
          ...
          <button type="button" className="ml-3 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200 hover:text-gray-900" onClick={() => setSidebarOpen(false)}><span className="sr-only">Close menu</span><X className="h-5 w-5" aria-hidden="true" /></button>
        </div>
        ...
        <div className="safe-bottom border-t border-gray-100 p-4"><div className="flex items-center justify-between rounded-xl bg-gray-50 p-3"><div className="min-w-0 pr-2"><p className="truncate text-sm font-bold text-gray-800">{session?.user?.name || "User"}</p><p className="truncate text-xs font-medium capitalize text-gray-500">{session?.user?.role || "Admin"}</p></div><button onClick={() => signOut({ callbackUrl: "/login" })} className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-white text-gray-400 shadow-sm hover:bg-red-50 hover:text-red-600"><LogOut className="h-5 w-5" /></button></div></div>
      </div>
    </div>
    ...
    <div className="min-w-0 flex flex-1 flex-col overflow-hidden">
      ...
      <div className="safe-top flex min-h-14 items-center justify-between border-b border-gray-200 bg-white/95 px-3 shadow-sm backdrop-blur lg:hidden">
        ...
        <button type="button" className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-gray-50 text-gray-600 hover:bg-gray-100 hover:text-gray-900" onClick={() => setSidebarOpen(true)}><span className="sr-only">Open menu</span><Menu className="h-5 w-5" aria-hidden="true" /></button>
      </div>
      ...
      <main className="relative z-0 flex-1 overflow-y-auto pb-24 focus:outline-none lg:pb-0"><div className="py-3 sm:py-6"><div className="mx-auto max-w-7xl px-3 sm:px-6 md:px-8 page-transition">{children}</div></div></main>
    </div>
    ...
    <nav className="safe-bottom fixed bottom-0 z-40 flex w-full items-center border-t border-gray-200 bg-white/95 px-1 pt-1 shadow-[0_-4px_18px_rgba(15,23,42,0.08)] backdrop-blur lg:hidden" aria-label="Mobile navigation">
      {mobileNavigation.map((item) => {
        ...
        return <Link key={item.name} href={item.href} className={clsx(active ? "text-blue-600" : "text-gray-500 hover:text-gray-900", "flex min-h-14 flex-1 flex-col items-center justify-center rounded-xl px-1 py-1 text-[10px] font-semibold transition-colors")}><span className={clsx(active ? "bg-blue-50" : "bg-transparent", "mb-0.5 flex h-7 w-10 items-center justify-center rounded-full transition-colors")}><item.icon className="h-[19px] w-[19px]" aria-hidden="true" /></span>{item.name}</Link>;
      })}
      <button type="button" onClick={() => setSidebarOpen(true)} className="flex min-h-14 flex-1 flex-col items-center justify-center rounded-xl px-1 py-1 text-[10px] font-semibold text-gray-500 transition-colors hover:text-gray-900"><span className="mb-0.5 flex h-7 w-10 items-center justify-center rounded-full"><Menu className="h-[19px] w-[19px]" /></span>More</button>
    </nav>
  </div>;
}

...
import { LayoutDashboard, Users, GraduationCap, CreditCard, CalendarCheck, Settings, LogOut, Menu, X, Search, PieChart, Download, ChevronLeft, Wallet, StickyNote } from "lucide-react";
...
const navigation = [
  ...
  { name: "Notes", href: "/dashboard/notes", icon: StickyNote, pwaOnly: true },
...
const isStandalonePWA = () => typeof window !== "undefined" && (window.matchMedia?.("(display-mode: standalone)")?.matches || window.navigator.standalone === true);
...
const pageMeta = [["/dashboard/students", "Students"], ["/dashboard/fees", "Fees & Payments"], ["/dashboard/daily-expense", "Daily Expense"], ["/dashboard/attendance", "Attendance"], ["/dashboard/notes", "Notes"], ["/dashboard/batches", "Batches"], ["/dashboard/reports", "Reports"], ["/dashboard/settings", "Settings"]];
...
export default function DashboardLayout({ children }) {
  const [sidebarOpen, setSidebarOpen] = useState(false), [centerName, setCenterName] = useState("Aminul Islam"), [searchQuery, setSearchQuery] = useState(""), [isPWA, setIsPWA] = useState(false);
  ...
  const visibleNavigation = navigation.filter(item => !item.pwaOnly || isPWA);
  ...
  useEffect(() => { mobileNavigation.forEach(item => router.prefetch(item.href)); router.prefetch("/dashboard/batches"); router.prefetch("/dashboard/reports"); router.prefetch("/dashboard/daily-expense"); if (isPWA) router.prefetch("/dashboard/notes"); }, [router, isPWA]);
  ...
  return <div className="min-h-dvh bg-gray-100 flex">
    <div className={clsx("fixed inset-0 z-50 lg:hidden", sidebarOpen ? "pointer-events-auto" : "pointer-events-none")} role="dialog" aria-modal="true" aria-hidden={!sidebarOpen}>
      ...
      <div className={clsx("relative flex h-full w-[min(86vw,320px)] flex-col bg-white pt-[max(1rem,env(safe-area-inset-top))] pb-4 shadow-2xl transition-transform duration-200 ease-out", sidebarOpen ? "translate-x-0" : "-translate-x-full")}>
        <div className="flex flex-shrink-0 items-center justify-between px-5 pb-5"><div className="flex min-w-0 items-center"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 shadow-lg shadow-blue-200/40"><GraduationCap className="h-6 w-6 text-white" /></div><span className="ml-3 truncate text-lg font-bold text-gray-900">{centerName}</span></div><button type="button" className="ml-3 flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 text-gray-500" onClick={() => { haptic(); setSidebarOpen(false); }}><X className="h-5 w-5" /></button></div>
        <div className="h-px bg-gray-100" /><div className="mt-4 flex-1 overflow-y-auto px-3"><nav className="space-y-1.5">{visibleNavigation.map(item => { const active = pathname === item.href || (item.href !== "/dashboard" && pathname.startsWith(`${item.href}/`)); return <Link key={item.name} href={item.href} onClick={() => haptic()} className={clsx(active ? "pwa-menu-item-active text-white shadow-md shadow-blue-200/40" : "text-slate-600 hover:bg-blue-50/70 hover:text-blue-700", "pwa-menu-item group flex min-h-12 items-center rounded-2xl px-4 py-3 text-[15px] font-semibold transition-all duration-200")}><item.icon className={clsx(active ? "text-white" : "text-gray-400", "mr-4 h-5 w-5")} />{item.name}</Link>; })}<InstallButton mobile /></nav></div>
    ...
    <nav className="safe-bottom fixed bottom-0 left-2 right-2 z-40 flex items-center rounded-3xl border border-slate-200/80 bg-white/94 px-1.5 pt-1.5 shadow-[0_-8px_30px_rgba(15,23,42,0.10),0_8px_30px_rgba(15,23,42,0.06)] backdrop-blur-2xl lg:hidden" aria-label="Mobile navigation">{mobileNavigation.map(item => { const active = pathname === item.href || (item.href !== "/dashboard" && pathname.startsWith(`${item.href}/`)); return <Link key={item.name} href={item.href} prefetch onClick={() => haptic()} aria-current={active ? "page" : undefined} className={clsx(active ? "text-blue-700" : "text-slate-500", "native-tab group relative flex min-h-14 flex-1 flex-col items-center justify-center rounded-2xl px-1 py-1 text-[10px] font-semibold active:scale-95 active:opacity-70")}><span className={clsx("native-tab-icon mb-0.5 flex h-8 w-11 items-center justify-center rounded-xl transition-all duration-200", active && "native-tab-icon-active")}><item.icon className="h-[19px] w-[19px]" /></span>{item.name}</Link>; })}</nav>

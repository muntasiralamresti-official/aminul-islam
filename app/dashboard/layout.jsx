...
import { LayoutDashboard, Users, GraduationCap, CreditCard, CalendarCheck, Settings, LogOut, Menu, X, Search, PieChart, Download, ChevronLeft } from "lucide-react";
...
const pageMeta = [
  ["/dashboard/students", "Students"], ["/dashboard/fees", "Fees & Payments"], ["/dashboard/attendance", "Attendance"],
  ["/dashboard/batches", "Batches"], ["/dashboard/reports", "Reports"], ["/dashboard/settings", "Settings"],
];
function getPageTitle(pathname) {
  if (pathname === "/dashboard") return "Overview";
  const match = pageMeta.find(([path]) => pathname === path || pathname.startsWith(`${path}/`));
  return match?.[1] || "Aminul Islam";
}
...
export default function DashboardLayout({ children }) {
  ...
  const isStudents = pathname === "/dashboard/students";
  ...
  useEffect(() => {
    ...
    if (cachedName) { setCenterName(cachedName); return; }
    fetch("/api/settings", { cache: "no-store" }).then((res) => res.ok ? res.json() : null).then((data) => {
      if (data?.centerName) { setCenterName(data.centerName); sessionStorage.setItem("aminul-islam-center-name", data.centerName); }
    }).catch(() => {});
  }, []);
  useEffect(() => {
    mobileNavigation.forEach((item) => router.prefetch(item.href));
    router.prefetch("/dashboard/batches"); router.prefetch("/dashboard/reports");
  }, [router]);
  useEffect(() => { setSidebarOpen(false); }, [pathname]);

  const handleSearch = (event) => {
    event.preventDefault(); const query = searchQuery.trim(); haptic();
  ...
  return <div className="min-h-dvh bg-gray-100 flex">
    ...
    <div className="min-w-0 min-h-0 flex flex-1 flex-col overflow-hidden">
      <div className="safe-top flex min-h-14 flex-shrink-0 items-center border-b border-gray-200 bg-white/95 px-3 shadow-sm backdrop-blur lg:hidden">
        {isStackRoute ? <button type="button" onClick={() => { haptic(); router.back(); }} className="mr-2 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-gray-50 text-gray-600"><ChevronLeft className="h-5 w-5" /></button> : <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl bg-blue-600 shadow-sm"><GraduationCap className="h-5 w-5 text-white" /></div>}
        <div className="min-w-0 flex-1 px-2"><p className="truncate text-[10px] font-semibold uppercase tracking-wider text-blue-600">{centerName}</p><p className="truncate text-base font-bold tracking-tight text-gray-900">{pageTitle}</p></div>
        <button type="button" className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-gray-50 text-gray-600" onClick={() => { haptic(); setSidebarOpen(true); }}><Menu className="h-5 w-5" /></button>
      </div>
    ...
    <style jsx global>{`
      @media (max-width: 640px) {
        ...
        main { overflow-y: auto !important; overscroll-behavior-y: auto !important; touch-action: pan-y; }
        main, main * { pointer-events: auto; }

        /* The old generic table-card rules made Fees look like a compressed legacy table. */
        ...
        .page-transition .overflow-x-auto:has(table thead th:nth-child(10)) tbody td:nth-child(4),
        .page-transition .overflow-x-auto:has(table thead th:nth-child(10)) tbody td:nth-child(5),
        .page-transition .overflow-x-auto:has(table thead th:nth-child(10)) tbody td:nth-child(6),
        .page-transition .overflow-x-auto:has(table thead th:nth-child(10)) tbody td:nth-child(7),
        .page-transition .overflow-x-auto:has(table thead th:nth-child(10)) tbody td:nth-child(9) { font-weight: 750; }

        /* Payment history gets a separate compact transaction-card treatment. */
        ...
        /* Keep monthly attendance/report calendar tables horizontally scrollable. */

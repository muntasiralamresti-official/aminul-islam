    ...
    <div className="mx-auto w-full max-w-4xl pb-8">
      ...
      <section className="overflow-hidden rounded-[26px] border border-slate-200/90 bg-white shadow-[0_10px_35px_rgba(15,23,42,0.09)] no-print">
        ...
        <div className="border-t border-slate-100 p-4 sm:p-5">
          <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-slate-500" /><span className="font-semibold text-slate-900">Status</span></div><span className={`rounded-full px-4 py-2 text-sm font-bold uppercase ${isActive ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{student.status}</span></div>
        </div>
      </section>

      <section className="mt-4 rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm sm:mt-5 sm:p-5 no-print">
        <p className="mb-3 text-base font-semibold text-slate-900">Actions</p>
        <div className="grid grid-cols-3 gap-2.5 sm:gap-3">
          <div className="flex min-h-20 flex-col items-center justify-center rounded-2xl bg-blue-50 text-blue-700"><Eye className="h-6 w-6" /><span className="mt-1 text-sm font-semibold">View</span></div>
          <Link href={`/dashboard/students/${student._id}/edit`} className="flex min-h-20 flex-col items-center justify-center rounded-2xl bg-blue-600 text-white shadow-sm hover:bg-blue-700"><Edit className="h-6 w-6" /><span className="mt-1 text-sm font-semibold">Edit</span></Link>
          <Link href={`/dashboard/fees?student=${student._id}`} className="flex min-h-20 flex-col items-center justify-center rounded-2xl bg-emerald-50 text-emerald-700 hover:bg-emerald-100"><WalletIcon /><span className="mt-1 text-sm font-semibold">Fees</span></Link>
        </div>
      </section>
          ...
          <div id="student-id-card-pdf" className="id-card-preview w-full max-w-[430px] overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-lg">
            ...
            <div className="p-5">
              ...
              <div className="mt-5 grid grid-cols-2 gap-2.5">
                <IdField label="Class" value={student.classLevel || "N/A"} />
                <IdField label="Batch" value={student.batch?.name || "N/A"} />
                <IdField label="Subject" value={student.batch?.subject || "N/A"} />
                <IdField label="Phone" value={student.phone || "N/A"} />
              </div>
              <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-4"><div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Admission Date</p><p className="mt-1 text-xs font-semibold text-slate-700">{student.admissionDate ? new Date(student.admissionDate).toLocaleDateString("en-BD") : "N/A"}</p></div><div className="flex h-14 w-14 items-center justify-center rounded-xl border-2 border-dashed border-slate-300 text-[9px] font-bold text-slate-400">QR<br />CODE</div></div>
            </div>
          </div>
        </div>
      ...
      <style jsx global>{`
        .print-only-id-card { display: none; }
        @media print {
          @page { size: A4 portrait; margin: 12mm; }
          body * { visibility: hidden !important; }
          .print-only-id-card, .print-only-id-card * { visibility: visible !important; }
          .print-only-id-card { display: flex !important; position: fixed; inset: 0; align-items: flex-start; justify-content: center; padding-top: 20mm; }
          .id-card-print { width: 86mm; border: 1px solid #dbe3ef; border-radius: 6mm; overflow: hidden; background: white; box-shadow: none; }
...
function InfoRow({ icon: Icon, label, value, action }) {
  return <div className="flex min-h-[68px] items-center gap-3 py-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-slate-100 text-slate-500"><Icon className="h-5 w-5" /></div><div className="min-w-0 flex-1"><p className="text-xs font-medium text-slate-500">{label}</p><p className="mt-0.5 truncate text-[15px] font-medium text-slate-950">{value}</p></div>{action}</div>;
}

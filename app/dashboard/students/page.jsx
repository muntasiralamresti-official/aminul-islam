...
function StudentsContent() {
    ...
    try {
      ...
      try {
        Object.keys(sessionStorage)
          .filter((key) => key.startsWith("aminul-islam-students-"))
          .forEach((key) => sessionStorage.removeItem(key));
      } catch {}
      if (students.length === 1 && currentPage > 1) handlePageChange(currentPage - 1);
      else fetchStudents({ force: true });
    } catch (deleteError) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-6 w-full overflow-hidden rounded-xl shadow-sm ring-1 ring-black/5 sm:mt-8">
...
export default function StudentsPage() {
  return <Suspense fallback={<TableSkeleton columns={6} rows={7} />}><StudentsContent /></Suspense>;
}

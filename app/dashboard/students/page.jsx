...
const getCacheKey = (page, search, limit) => `aminul-islam-students-${page}-${search}-${limit}`;
...
function StudentsContent() {
  ...
  const prefetchNextPage = useCallback(async (page, search, totalPagesValue) => {
    ...
    const key = getCacheKey(nextPage, search, pageSize);
    if (prefetchRef.current.has(key)) return;
    prefetchRef.current.add(key);
    try {
      if (pageSize === "all") return; const res = await fetch(`/api/students?page=${nextPage}&limit=${pageSize}`, { cache: "no-store" });
      if (!res.ok) return;
  ...
  const fetchStudents = useCallback(async ({ force = false } = {}) => {
    const cacheKey = getCacheKey(currentPage, searchQuery, pageSize);
    ...
    setError("");
    try {
      const params = new URLSearchParams(); if (pageSize === "all") { params.set("all", "true"); } else { params.set("page", String(currentPage)); params.set("limit", String(pageSize)); }
      if (searchQuery) params.set("search", searchQuery);
      const res = await fetch(`/api/students?${params.toString()}`, { cache: "no-store" });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed to load students");
      const result = Array.isArray(data) ? { students: data, total: data.length, totalPages: 1 } : { students: data.students || [], total: data.total || 0, totalPages: data.totalPages || 1 };
  ...
  }, [currentPage, searchQuery, prefetchNextPage, saveCache, pageSize]);

  useEffect(() => { fetchStudents(); }, [fetchStudents]);

  const refreshStudents = useCallback(async () => {
    if (refreshing) return;
    try { sessionStorage.removeItem(getCacheKey(currentPage, searchQuery, pageSize)); } catch {}
    hasDataRef.current = false;
    await fetchStudents({ force: true });
  }, [currentPage, searchQuery, pageSize, refreshing, fetchStudents]);
  ...
  const firstResult = total === 0 ? 0 : pageSize === "all" ? 1 : (currentPage - 1) * pageSize + 1;
  const lastResult = pageSize === "all" ? total : Math.min(currentPage * pageSize, total);
  ...
  return (
    <div className="w-full pb-4" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} style={{ touchAction: pulling ? "none" : "pan-y" }}>
      ...
      <div className="mt-5 rounded-2xl border border-gray-200 bg-white p-3 shadow-sm sm:p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center"><input type="search" inputMode="search" placeholder="Search students..." value={searchInput} onChange={(event) => setSearchInput(event.target.value)} className="min-h-11 flex-1 rounded-xl border border-gray-200 bg-gray-50 px-4 text-sm outline-none focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100" /><div className="flex items-center justify-between gap-3 text-xs text-gray-500"><span>{firstResult}–{lastResult} of {total}</span><select value={pageSize} onChange={(e) => { const v = e.target.value === "all" ? "all" : Number(e.target.value); setPageSize(v); setCurrentPage(1); }} className="rounded-lg border border-gray-200 bg-white px-2 py-2 text-xs"><option value={10}>10</option><option value={20}>20</option><option value={50}>50</option><option value="all">All</option></select></div></div></div>

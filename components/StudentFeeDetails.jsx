...
export default function StudentFeeDetails({ studentId, initialYear = new Date().getFullYear(), onClose }) {
  ...
  const modal = (
    <div className="fixed inset-0 z-[200] overflow-y-auto bg-slate-950/45 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Student fee details">
      <div className="flex min-h-full items-start justify-center p-0 sm:items-center sm:p-5">
        <div className="ui-modal-panel relative flex max-h-[100dvh] min-h-screen w-full max-w-5xl flex-col overflow-hidden bg-slate-50 shadow-2xl sm:max-h-[calc(100dvh-2.5rem)] sm:min-h-0 sm:rounded-[28px]">
          <header className="flex flex-shrink-0 items-center justify-between border-b border-slate-200/80 bg-white px-4 py-4 sm:px-6">
            <div className="flex min-w-0 items-center gap-3 sm:gap-4"><button onClick={onClose} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-slate-100 text-slate-600 hover:bg-slate-200" aria-label="Close"><X className="h-5 w-5" /></button><div className="min-w-0"><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-blue-600">Fee & Payment</p><h2 className="truncate text-xl font-bold tracking-tight text-slate-950 sm:text-2xl">{data?.student?.name || "Student"}</h2></div></div>
